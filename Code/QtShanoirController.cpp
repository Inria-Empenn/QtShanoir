...
void
QtShanoirController::attachTreeWidget(QtShanoirTreeWidget * widget)
{
    d->tree = widget;
    if (d->tree) {
...
void
QtShanoirController::getError(QString xmlserial)
{
    //    qDebug() << xmlserial;
...
void
QtShanoirController::sslErrors(const QList<QSslError> & errors)
{
    if (errors.size() == 0)
        return;
...
void
QtShanoirController::getResponse()
{
...
void
QtShanoirController::setLogin(QString key, QString ws, QString impl)
{
...
void
QtShanoirController::populateQueries()
{
    ...
    d->perWsQuery.clear();
    {
        ...
        find->WsImpl = impl;
        find->WsMethod = "find";
        //        QObject::connect(find, SIGNAL(response(QString)), this, SLOT(parseStudy(QString)));
        if (d->tree)
...
void
QtShanoirController::mrExamQuery(QString str)
{
...
void
QtShanoirController::setMrExamQuery(QString key, QString id)
{
    ...
    find->WsMethod = "find";
    //    QObject::connect(find, SIGNAL(response(QString)), this, SLOT(parseMrExamination(QString)));
    if (d->tree)
...
void
QtShanoirController::datasetQuery(QString str1, QString str2)
{
...
void
QtShanoirController::setDatasetQuery(QString key, QString id, QString exId)
{
    ...
    QtShanoirWsQuery* find = new QtShanoirWsQuery(webs, impl);
    find->WsMethod = "find";
    //    QObject::connect(find, SIGNAL(response(QString)), this, SLOT(parseAcquisition(QString)));
    if (d->tree)
...
void
QtShanoirController::currentId(int id)
{
    d->curId = id;
}

void
QtShanoirController::download(QString xmlserial)
{
    qDebug() << xmlserial;
    const QtSoapMessage &message = d->http.getResponse();
...
void
QtShanoirController::setDownload(QString key, QString id)
{
    qDebug() << "Prepare WS query for Id" << id;
